        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-6xl mx-auto">
          {displayPositions.map((position, index) => {
            return (
              <div
                key={index}
                className="bento-card group hover:-translate-y-1 transition-all"
              >
                ...
                <ul className="list-none p-0 mb-4 space-y-1.5">
                  {position.highlights.slice(0, 2).map((highlight, idx) => (
                    <li
                      key={idx}
                      className="text-xs text-text-tertiary leading-relaxed flex items-start gap-2"
                    >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {position.techStack.map((tech) => (
                    <span
                      key={tech}
                      className="px-2 py-1 bg-bg-tertiary text-text-secondary rounded-lg text-[10px] font-medium border border-border-color"
                    >
                      {tech}
                    </span>
                  ))}
