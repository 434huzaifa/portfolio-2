...
### 1. **JSON Content File** (`portfolio-data.ts`)
...
Edit the `sectionOrder` array in `portfolio-data.ts`:
```json
"sectionOrder": [
  "hero",
  "about", 
...
Replace these placeholder links in `portfolio-data.ts`:
...
### Content:
- `portfolio-data.ts` - **MAIN FILE** - All your content
